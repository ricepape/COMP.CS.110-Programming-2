...
int main() {
    ...
    while ( true ) {
        ...
        for (unsigned int i = 0; i < command_to_be_executed.length(); i++) {
            ...
            char ch = toupper(command_to_be_executed[i]);

            command+=ch;
          }
        int count=0;
        for (auto& element : COMMANDS){
            if (element.str==command){
                count=1;
                if(element.parameter_number==0 && pieces.size()==1){
                    cout << GREETING_AT_END << endl;
                    return EXIT_SUCCESS;
                }
                else if (element.parameter_number!=pieces.size()-1){
                    cout << "Error: wrong number of parameters." << endl;
                    continue;
                } else {
                    double left,right;
                    bool first_element=string_to_double(pieces.at(1),left);
                    bool second_element=string_to_double(pieces.at(2),right);
                    if (first_element==false or second_element==false){
                        cout << "Error: a non-number operand."<< endl;
                        continue;
                    } else {
                        cout << element.action(left,right) << endl;
                    }
                }
            }
        }
